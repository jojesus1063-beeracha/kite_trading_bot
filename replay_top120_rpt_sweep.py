...
margin_budget = (
    AVAILABLE_MARGIN * MAX_POSITION_PCT / 100.0
)

max_daily_loss = (
    CAPITAL * MAX_DAILY_LOSS_PCT / 100.0
)

for rpt_pct in RPT_LEVELS:
    ...
    if not t.empty:
        ...
        # Drawdown from chronological NET P&L
        equity = 0.0
        peak = 0.0
        max_dd = 0.0
    ...
    all_summary.append({
        ...
        "max_drawdown": max_dd,
        "blocked": len(b),
        "zero_qty": reason_counts.get(
            "ZERO_QTY", 0
        ),
        "daily_risk_blocked": reason_counts.get(
            "DAILY_RISK_BLOCK", 0
        ),
        "no_margin": reason_counts.get(
            "NO_MARGIN", 0
        ),
    })

    if not t.empty:
        all_trade_rows.append(t)

    if not b.empty:
        all_blocked_rows.append(b)


summary = pd.DataFrame(all_summary)

summary.to_csv(
    OUT / "rpt_sweep_summary.csv",
    index=False
)
...
print(
    "Max position:",
    f"{MAX_POSITION_PCT:.0f}%"
)
print(
    "Daily loss:",
    f"{MAX_DAILY_LOSS_PCT:.2f}% = "
    f"Rs {max_daily_loss:.2f}"
)
print(
    "Signals available:",
    len(signals)
