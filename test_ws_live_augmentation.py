...
def make_rest_df(last_date, n=5, interval_minutes=5, tz=None):
    rows = []
    for i in range(n):
...
result_df, augmented = engine.get_augmented_candles("SYM", "5minute", df)
...
engine.candle_builders_5m["SYM"].finalized.append({
    "date": last_date + timedelta(minutes=5),
...
pd.Timestamp.now = staticmethod(lambda tz=None: pd.Timestamp(last_date + timedelta(minutes=5, seconds=10), tz=tz))
...
engine2.candle_builders_5m["SYM2"].finalized.append({
    "date": last_date + timedelta(minutes=10),  # skipped a candle -- gap
    "open": 100.5, "high": 102, "low": 100, "close": 101.5, "volume": 1200,
})
df2 = make_rest_df(last_date)
result_df2, augmented2 = engine2.get_augmented_candles("SYM2", "5minute", df2)
...
engine3.candle_builders_5m["SYM3"].finalized.append({
    "date": last_date + timedelta(minutes=5),
    "open": 100.5, "high": 102, "low": 100, "close": 101.5, "volume": 1200,
})
...
result_df3, augmented3 = engine3.get_augmented_candles("SYM3", "5minute", df3)
pd.Timestamp.now = orig_now
...
result_df4, augmented4 = engine.get_augmented_candles("SYM", "5minute", empty_df)
check("Empty REST df -> returned unmodified, no crash", augmented4 is False)

# -- Case 6: unknown symbol -> unmodified, no crash ------------------------

result_df5, augmented5 = engine.get_augmented_candles("NOT_A_SYMBOL", "5minute", df)
...
engine5.candle_builders_5m["SYM5"].finalized.append({
    "date": datetime(2026, 8, 5, 9, 35),  # naive -- as Kite ticks actually arrive
    "open": 100.5, "high": 102, "low": 100, "close": 101.5, "volume": 1200,
})
pd.Timestamp.now = staticmethod(lambda tz=None: pd.Timestamp(datetime(2026, 8, 5, 9, 35, 5), tz=tz))
result_df7, augmented7 = engine5.get_augmented_candles("SYM5", "5minute", df_tz)
pd.Timestamp.now = orig_now
