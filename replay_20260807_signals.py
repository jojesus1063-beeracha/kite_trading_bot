...
def main():
    ...
    print("RESULT")
    print("=" * 88)
    print(f"Raw strategy signals (after trend/ADX + 5m EMA/volume + VWAP acceptance + full EMA200): {len(raw_strategy_signals)}")
    ...
    if raw_strategy_signals:
        ...
        print("\nNo raw strategy signals were generated by the corrected strategy for the production shortlist.")

    if gate_passed:
        print("\nSURVIVED EMA200-DIRECTIONAL + RVOL")
        for item in sorted(gate_passed, key=lambda x: x["timestamp"]):
            rv = "N/A" if item["rvol"] is None else f"{item['rvol']:.2f}"
            print(f"{item['timestamp']:%H:%M}  {item['symbol']:<14} {item['direction']:<4} "
                  f"entry={item['entry']:.2f} rvol={rv} ema200={item['ema200_eligibility']}")
    else:
        print("\nNo corrected strategy signal survived both the EMA200 directional gate and RVOL gate.")

    print("\nCOUNTS")
    for reason, count in rejection_counts.most_common():
        print(f"{count:5d}  {reason}")

    print("\nIMPORTANT: this answers historical signal eligibility. It does not simulate live-price validation, "
          "candidate ranking across simultaneous symbols, broker fills, slippage, position limits, or P&L.")
