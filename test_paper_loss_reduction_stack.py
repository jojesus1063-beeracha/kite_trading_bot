...
def test_current_paper_policy_constants():
    ...
    assert risk_launcher.PAPER_ADX_BLOCK_HIGH == 0.0
    assert risk_launcher.PAPER_ADX_NORMAL_FROM == 20.0
...
def test_risk_overrides_disable_frequency_caps(monkeypatch):
    ...
    assert cfg.PAPER_ADX_BLOCK_LOW == cfg.PAPER_ADX_BLOCK_HIGH == 0.0
...
def test_adx_is_direction_only_and_never_blocks(monkeypatch):
    monkeypatch.setattr(cfg, "PAPER_TRADING", True, raising=False)
    risk_launcher.apply_paper_risk_overrides()
    risk_launcher.install_direction_only_adx_policy()

    assert contrarian._adx_entry_blocked(None) is False
    assert contrarian._adx_entry_blocked(10.0) is False
...
def test_adx_lt20_reverse_gte20_normal(monkeypatch):
    monkeypatch.setattr(cfg, "PAPER_TRADING", True, raising=False)
    risk_launcher.apply_paper_risk_overrides()
    risk_launcher.install_direction_only_adx_policy()
    ...
    direction_19, _, _ = contrarian.ema_direction(df, adx=19.99)
    direction_20, _, _ = contrarian.ema_direction(df, adx=20.0)
    direction_35, _, _ = contrarian.ema_direction(df, adx=35.0)

    assert direction_19 == "SELL"  # reversed interpretation
    assert direction_20 == "BUY"   # normal interpretation
    assert direction_35 == "BUY"   # normal interpretation
