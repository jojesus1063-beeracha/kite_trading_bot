...
tests=(
  ...
  test_exit_order_verification.py
  test_normal_exit_integration.py
  test_exit_recovery.py
)
...
for test_file in "${tests[@]}"; do
    ...
    # Some legacy test scripts print failures but incorrectly
    # return process exit code 0. Treat visible failure markers as
    # genuine failures. The documented test_exit_stack baseline is
    # handled above and continues before reaching this check.
    if grep -Eq \
       'FAIL:|CRASHED:|Results: [0-9]+ passed, [1-9][0-9]* failed|FINAL Results: [0-9]+ passed, [1-9][0-9]* failed' \
       "$output_file"; then
        rc=1
    fi
