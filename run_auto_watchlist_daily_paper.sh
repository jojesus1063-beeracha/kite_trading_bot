#!/usr/bin/env bash
# Paper-mode counterpart to run_auto_watchlist_daily.sh. The live-mode
# script is deliberately left untouched -- this is a SEPARATE file so
# the live-only safety gates in that script are never weakened or
# bypassed. This script has the exact opposite polarity on every
# paper_trading check: it requires PAPER mode, and refuses to run in
# live mode, mirroring the live script's own discipline in reverse.
set -Eeuo pipefail

PROJECT="/home/ubuntu/kite_trading_bot"
PYTHON="$PROJECT/venv/bin/python3"
LOCK_DIR="$PROJECT/runtime/auto_watchlist"
LOCK_FILE="$LOCK_DIR/daily_paper.lock"
...
if [[ "${ALLOW_PAPER_AUTOSTART:-}" != "YES" ]]; then
    echo "SAFETY BLOCK: paper automatic startup is not authorised."
    exit 1
fi

echo "Paper watchlist generation started:"
...
if data.get("paper_trading") is not True:
    raise SystemExit(
        "SAFETY BLOCK: configuration is not in paper mode. "
        "This script refuses to run against a live-mode configuration -- "
        "use run_auto_watchlist_daily.sh (the separate, live-only script) "
        "for that instead."
    )
...
print("PASS: paper-mode safety limits validated")
...
if config.get("paper_trading") is not True:
    raise SystemExit(
        "FAIL: configuration is no longer in paper mode."
    )
...
echo "Paper watchlist generation completed:"
TZ=Asia/Kolkata date
