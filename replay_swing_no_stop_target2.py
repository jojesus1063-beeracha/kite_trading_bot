...
OUTPUT = Path(
    "runtime/swing_dynamic_target_all_days_20260816.json"
)
...
def target_scenario(entry, future):
    scenario = "DYNAMIC_TARGET_2_PER_DAY_MAX_5D"
    ...
    for session_number, candle in enumerate(
        selected,
        start=1,
    ):
        target_pct = TARGET_PCT * session_number

        target_price = entry["entry"] * (
            1 + target_pct / 100
        )

        # If the session opens above that day's target,
        # exit at the available opening price.
        if candle["open"] >= target_price:
            return make_result(
                ...
                candle["open"],
                (
                    f"TARGET_GAP_DAY_{session_number}_"
                    f"{target_pct:.0f}PCT"
                ),
                horizon_available,
            )
        ...
        if candle["high"] >= target_price:
            return make_result(
                ...
                target_price,
                (
                    f"TARGET_DAY_{session_number}_"
                    f"{target_pct:.0f}PCT"
                ),
                horizon_available,
            )
...
scenarios = [
    ...
    "CLOSE_3D",
    "CLOSE_5D",
    "DYNAMIC_TARGET_2_PER_DAY_MAX_5D",
]
...
report = {
    "method": {
        ...
        "initial_target_pct": TARGET_PCT,
        "target_policy": (
            "DAY_1_2PCT_DAY_2_4PCT_DAY_3_6PCT_"
            "DAY_4_8PCT_DAY_5_10PCT"
        ),
        "stop_loss": None,
