...
def _passes_entry_continuation(symbol: str, df_15m: pd.DataFrame, df_5m: pd.DataFrame, direction: str, cfg) -> bool:
    ...
    result = assess_entry_continuation(
        ...
        direction,
        cfg,
        vwap_reference=_current_vwap_reference(df_15m, df_5m.iloc[-1]["date"]),
    )
    if not result.accepted:
        logger.info(f"{symbol}: ENTRY CONTINUATION | REJECT | {result.reason} | {result.detail}")
        mark_filter_status(symbol, "ENTRY_CONTINUATION", detail={"direction": direction, "reason": result.reason, **result.detail})
        return False
...
def evaluate(symbol: str, df_15m: pd.DataFrame, df_5m: pd.DataFrame, cfg) -> Optional[Signal]:
    ...
    if trend is None:
        mark_filter_status(symbol, "TREND_OR_ADX", detail={"reason": "15m EMA/VWAP trend or binary ADX requirement not satisfied"})
        return None
    if pd.isna(curr["avg_volume"]) or pd.isna(curr["ema_entry"]):
        mark_filter_status(symbol, "ENTRY_DATA", detail={"reason": "avg_volume or entry EMA unavailable"})
        return None

    confidence = latest_completed_15m_confidence(df_15m, curr["date"], cfg)
    if resolve_adx_mode(cfg) == "dynamic" and confidence == "REJECTED":
        mark_filter_status(symbol, "TREND_OR_ADX", detail={"reason": "dynamic ADX confidence rejected trend"})
        return None

    volume_ok = curr["volume"] > curr["avg_volume"] * cfg.VOLUME_MULTIPLIER
    continuation_enabled = getattr(cfg, "ENABLE_ENTRY_CONTINUATION_FILTER", False)
    continuation_reason = " + entry continuation" if continuation_enabled else ""
    ...
    if trend == "UP" and curr["close"] > curr["ema_entry"] and volume_ok:
        ...
        if not _passes_vwap_acceptance(symbol, df_15m, df_5m, "BUY", cfg):
            mark_filter_status(symbol, "VWAP_ACCEPTANCE", detail={"direction": "BUY"})
            return None
        ...
        if ema200_status == "FAIL":
            ...
            mark_filter_status(symbol, "EMA200_CONFIRMATION", detail={"direction": "BUY", **(ema200_detail or {})})
            return None
        entry = curr["close"]
        stop = curr["low"] * (1 - cfg.SL_BUFFER_PCT / 100)
        risk = entry - stop
        if risk <= 0:
            mark_filter_status(symbol, "INVALID_RISK_GEOMETRY", detail={"direction": "BUY"})
            return None
        target = entry + risk * cfg.RISK_REWARD_MIN
        reason = f"15m uptrend + 5m close above EMA{cfg.ENTRY_EMA} on above-avg volume{continuation_reason} + VWAP acceptance"
        if getattr(cfg, "USE_ADX_FILTER", False):
            reason += " (ADX-confirmed trend)"
        if confidence:
            reason += f" [ADX confidence: {confidence}]"
        mark_filter_status(symbol, "STRATEGY_SIGNAL", detail={"direction": "BUY"})
    ...
    if trend == "DOWN" and curr["close"] < curr["ema_entry"] and volume_ok:
        ...
        if not _passes_vwap_acceptance(symbol, df_15m, df_5m, "SELL", cfg):
            mark_filter_status(symbol, "VWAP_ACCEPTANCE", detail={"direction": "SELL"})
            return None
        ema200_status, ema200_detail = evaluate_200ema_filter(df_15m, "SELL", cfg)
        if ema200_status == "FAIL":
            logger.info(format_rejection_log(symbol, ema200_status, ema200_detail))
            mark_filter_status(symbol, "EMA200_CONFIRMATION", detail={"direction": "SELL", **(ema200_detail or {})})
            return None
        entry = curr["close"]
        ...
        risk = stop - entry
        if risk <= 0:
            mark_filter_status(symbol, "INVALID_RISK_GEOMETRY", detail={"direction": "SELL"})
            return None
        target = entry - risk * cfg.RISK_REWARD_MIN
        reason = f"15m downtrend + 5m close below EMA{cfg.ENTRY_EMA} on above-avg volume{continuation_reason} + VWAP acceptance"
        if getattr(cfg, "USE_ADX_FILTER", False):
            reason += " (ADX-confirmed trend)"
        if confidence:
            reason += f" [ADX confidence: {confidence}]"
        mark_filter_status(symbol, "STRATEGY_SIGNAL", detail={"direction": "SELL"})
        return Signal(symbol, "SELL", entry, stop, target, curr["date"], reason, confidence=confidence)

    mark_filter_status(
        symbol,
        "ENTRY_EMA_OR_VOLUME",
        detail={"trend": trend, "volume_ok": bool(volume_ok), "close": float(curr["close"]), "ema_entry": float(curr["ema_entry"])},
    )
    return None
