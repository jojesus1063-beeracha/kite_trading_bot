...
for n, (_, row) in enumerate(
    ...
    # Prefer the exact rejected stock/date candles downloaded
    # specifically for this watchlist counterfactual.
    rejected_candle = Path(
        "runtime/watchlist_missed_opportunity/"
        "rejected_candles_3minute"
    ) / f"{date}_{symbol}.parquet"

    fallback_candle = (
        CANDLE_DIR / f"{symbol}.parquet"
    )

    if rejected_candle.exists():
        p = rejected_candle
    elif fallback_candle.exists():
        p = fallback_candle
    else:
        results.append({
            **row.to_dict(),
